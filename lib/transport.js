...
module.exports = class BLETransport extends ReadyResource {
  ...
  _log(...args) {
    if (this._debug) console.log('[ble]', ...args) // eslint-disable-line no-console
  }
  ...
  // After the radio returns, run the exact suspend/resume cycle the
  // app-level toggle exercises — the one recovery path proven to fully
  // restore advertising, scanning and dial state.
  _scheduleSelfHeal() {
    if (!this._radioWasDown) return
    if (this._healing || this.closing || this.closed || this._suspended) return
    this._radioWasDown = false
    this._healing = true
    this._log('radio recovered — cycling transport')
    this.suspend()
      .then(() => {
        this._healing = false
        if (!this.closing && !this.closed) this.resume()
      })
      .catch((err) => {
        this._healing = false
        safetyCatch(err)
