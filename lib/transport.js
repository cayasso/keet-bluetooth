...
module.exports = class BLETransport extends ReadyResource {
  ...
  _onRadioDown() {
    ...
    // NOTE: _serviceAdded stays true — there is no removeService API, and on
    // platforms where the GATT db survives a power cycle re-adding would
    // register a duplicate service; dialers then subscribe to the old
    // characteristic while we notify on the new one and every handshake dies.
    this._advertising = false
    this._scanning = false
